...
export type AuthFetchResult =
  ...
  | { kind: 'unreachable' } // fetch threw — API unreachable; session preserved (→ 503)
  | { kind: 'retryable' } // API answered 5xx — session preserved, show OfflineBanner (→ 502)
...
export async function authFetch(path: string, init?: ApiInit): Promise<AuthFetchResult> {
  ...
  if (outcome.status === 'unavailable') return { kind: 'unreachable' };
...
export type AuthPostResult =
  ...
  | { kind: 'reauth' }
  | { kind: 'unreachable' } // fetch threw — API unreachable (→ 503)
  | { kind: 'retryable' }; // API answered 5xx (→ 502)
