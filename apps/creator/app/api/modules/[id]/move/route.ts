...
// POST /api/modules/[id]/move — swap a module with its adjacent sibling (feature
// 0012 slice 2). The API answers 204. 400 (invalid_field, e.g. already at an
// edge) / 404 (module_not_found) pass through.
export async function POST(
  ...
  const result = await authFetch(`/modules/${encodeURIComponent(id)}/move`, {
    method: 'POST',
