// Fundi — shared module-cover generator. The creator picks ONE style (gradient
// or geometric) once for the whole program (ProgramSetup); every module then
// gets its own look deterministically from its position, no per-module effort
// and no image bytes. Used by the compact builder tree (small swatch) and the
// learner-facing preview (full card).
...
function Shape({ shape }) {
  ...
  ['top', 'left', 'right', 'bottom'].forEach((k) => { if (shape[k] !== undefined) pos[k] = shape[k]; });
  if (shape.kind === 'triangle') {
    return <div style={{ ...pos, width: 0, height: 0, borderLeft: shape.size * 0.56 + 'px solid transparent', borderRight: shape.size * 0.56 + 'px solid transparent', borderBottom: shape.size + 'px solid ' + shape.color, opacity: shape.opacity, transform: shape.rotate ? 'rotate(' + shape.rotate + 'deg)' : undefined }} />;
  }
  return <div style={{ ...pos, width: shape.size, height: shape.size, background: shape.color, opacity: shape.opacity, borderRadius: shape.kind === 'circle' ? '50%' : 12, transform: shape.rotate ? 'rotate(' + shape.rotate + 'deg)' : undefined }} />;
}

function ModuleCover({ coverStyle, seed = 0, height = 92, children, style }) {
  ...
  if (coverStyle === 'geometric') {
    return (
      <div style={{ position: 'relative', height, overflow: 'hidden', background: 'var(--base-ink-900)', ...style }}>
        {GEO_SHAPES[idx].map((s, i) => <Shape key={i} shape={s} />)}
        {children}
      </div>
    );
  }
...
if (typeof window !== 'undefined') { window.ModuleCover = ModuleCover; }
